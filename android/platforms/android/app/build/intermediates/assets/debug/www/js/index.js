...
function getCryptoPrices() {
  ...
  coins.forEach(coin => {
    // Make a GET request to the API endpoint for fetching the current value of each coin
    fetch(`http://wafer.revs.ir/api/current?crypto_symbol=${coin}&in_blu=${true}`)
      .then(response => response.json())
      .then(data => {
        const balanceElement = document.getElementById('balance-' + coin);
        balanceElement.textContent = `${data.blu} BluCoins`;
      })
      .catch(error => {
        console.error(`Error fetching ${coin} price:`, error);
        const listItem = document.createElement('li');
        listItem.textContent = `Error occurred while fetching ${coin} price.`;
